...
- [ ] **DeX's Recents button (|||) shows an empty screen** on MyDeX's invisible screen: Samsung's launcher creates no "desk" for scrcpy's virtual display (log: `isDexSpace: true`, `targetDeskId: -1`). It works with *Simulated display* (desk created). Fix idea: a scrcpy helper that creates the screen without the own-display-group/own-focus flags.
