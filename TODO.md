...
- [ ] **Known Samsung limitation: DeX's Recents button (|||) is empty on the invisible screen.** On scrcpy's virtual display DeX runs without its window mode – apps open full screen, and Recents (which lists DeX windows) only shows "add desktop". Tested and ruled out: scrcpy's display flags (own display group / own focus / device display group, with a rebuilt scrcpy helper), the phone being locked, restarting the Samsung launcher, MyDeX's options. On the *Simulated display* method DeX's window mode and Recents work (but Android shows a small copy on the phone). Untested idea: the "freeform windows" + "force desktop mode" developer options, which need a phone restart.
